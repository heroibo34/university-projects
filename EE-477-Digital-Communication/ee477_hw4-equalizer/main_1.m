...
for i=1:6    
    ...
    h=h_struct{temp};
    coeff_v{i}=zero_force_function(h,tap); 
end
...
for nEN=1:length(snr_db)    
    for i=1:6 % for 6 different cases
         ...
         for n=1:number_of_loop
             ...
             z=sign(conv(rec_sig,coeff_v{i},'same')); % decoded signal;
...
error_rate_matrix_1=error_rate_matrix;
save error_rate_matrix_1.mat error_rate_matrix_1
%%
load error_rate_matrix_1.mat
snr_db=[0 2 4 6 8 10 15 20 30 50 80 100];
last_error_rate_matrix_1=error_rate_matrix_1';
figure();
semilogy(snr_db,last_error_rate_matrix_1(:,1),'-x','DisplayName','9-tab & f1');hold on;
semilogy(snr_db,last_error_rate_matrix_1(:,2),'-x','DisplayName','9-tab & f2');hold on;
semilogy(snr_db,last_error_rate_matrix_1(:,3),'-x','DisplayName','9-tab & f3');hold on;
semilogy(snr_db,last_error_rate_matrix_1(:,4),'-x','DisplayName','15-tab & f1');hold on;
semilogy(snr_db,last_error_rate_matrix_1(:,5),'-x','DisplayName','15-tab & f2');hold on;
semilogy(snr_db,last_error_rate_matrix_1(:,6),'-x','DisplayName','15-tab & f3');hold on;
grid on;
legend;
